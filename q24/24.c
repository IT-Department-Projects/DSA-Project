...
int binary_search_least(long* array, long n , long k, long start, long end){
    ...
    if (start > end)	
		...
		else if (check_distance(array, n, k, mid-1) == 0)	
            return mid;
...
int main(){
    ...
    long num_temples; 
    long k; 
    long pos_array[100005]; 
    long opt_distance;

	scanf("%ld%ld", &num_temples, &k);
	
    for(long i=0; i<num_temples;i++)		
        scanf("%ld", pos_array+i);
	
    qsort(pos_array, num_temples, sizeof(long), longintcmp);
	
    if (check_distance(pos_array, num_temples, k, 1) == 1)	
        opt_distance = 1;
	else	
        opt_distance = binary_search_least(pos_array, num_temples, k, 2, pos_array[num_temples-1]);
